...
def test_onboarding_tour_status_is_persisted_per_user_and_version(wm_paths) -> None:
    ...
    first = client.get(
        "/api/v1/onboarding/tours/workflow-first-use?version=1", headers=_headers("root")
    )
    ...
    completed = client.put(
        "/api/v1/onboarding/tours/workflow-first-use",
    ...
    other_user = client.get(
        "/api/v1/onboarding/tours/workflow-first-use?version=1", headers=_headers("alice")
    )
    assert other_user.json()["should_show"] is True

    next_version = client.get(
        "/api/v1/onboarding/tours/workflow-first-use?version=2", headers=_headers("root")
    )
