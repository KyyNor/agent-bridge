...
async def test_agent_handler_prepends_skills_and_wraps_text():
    node = WorkflowGraph.model_validate({"nodes": [{"id": "a", "type": "agent", "name": "A", "position": {"x": 0, "y": 0}, "config": {"prompt": "review", "backend_key": "claude", "skill_names": ["review"]}}]}).nodes[0]
    agent = FakeAgent()
    result = await WorkflowNodeHandlers(agent_service=agent, scripts=SimpleNamespace(), skill_service=FakeSkills()).execute(node, NodeExecutionContext(actor="root", workflow={"workflow_key": "w", "profile_key": "p"}, run_id="r", input={}, task=None, nodes={}))
