...
def test_bare_browser_can_initialize_password_and_switch_to_admin(wm_paths) -> None:
    ...
    assert client.get("/api/v1/auth/admin/status").json() == {
        "configured": False,
        "active": False,
        "subject_user_id": None,
    }
    assert client.get("/api/v1/access/groups").status_code == 401

    login = client.post("/api/v1/auth/admin/session", json={"password": PASSWORD})
    ...
    assert client.get("/api/v1/access/groups").status_code == 200
    assert client.get("/api/v1/access/me").json()["is_maintenance_admin"] is True
...
def test_existing_identity_can_switch_without_replacing_sso_cookie(wm_paths) -> None:
    ...
    assert client.get(
        "/api/v1/access/me", headers={"X-Agent-Bridge-User": "alice"}
...
def test_configured_password_rejects_wrong_value_and_accepts_correct_value(wm_paths) -> None:
    ...
    assert first.post("/api/v1/auth/admin/session", json={"password": PASSWORD}).status_code == 200

    second = TestClient(app)
    assert second.post(
        "/api/v1/auth/admin/session", json={"password": "wrong-password"}
    ).status_code == 401
    assert second.get("/api/v1/access/groups").status_code == 401
    assert second.post(
        "/api/v1/auth/admin/session", json={"password": PASSWORD}
    ).status_code == 200
    assert second.get("/api/v1/access/groups").status_code == 200


def test_changing_password_invalidates_existing_admin_sessions(wm_paths) -> None:
    app = create_app(wm_paths, admins={"root"})
    client = TestClient(app)
    client.post("/api/v1/auth/admin/session", json={"password": PASSWORD})
    ...
    assert client.get("/api/v1/auth/admin/status").json()["active"] is False
    assert client.get("/api/v1/access/groups").status_code == 401
    assert client.post(
        "/api/v1/auth/admin/session", json={"password": PASSWORD}
    ).status_code == 401
    assert client.post(
        "/api/v1/auth/admin/session", json={"password": NEW_PASSWORD}
    ).status_code == 200
...
def test_password_admin_uses_existing_cross_group_data_bypass(wm_paths) -> None:
    ...
    assert setup.get(
        "/api/v1/kbs", headers={"X-Agent-Bridge-User": "bob"}
    ).json() == []

    password_admin = TestClient(app)
    password_admin.post("/api/v1/auth/admin/session", json={"password": PASSWORD})
    visible = password_admin.get("/api/v1/kbs")
