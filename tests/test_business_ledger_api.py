...
def test_business_ledger_management_api(wm_paths) -> None:
    ...
    with TestClient(app) as client:
        ...
        added = client.post("/api/v1/business-ledgers/assets/records", headers=headers, json={"values": {"name": "支付系统", "cost": 12}})
        assert added.status_code == 200
        found = client.post(
            "/api/v1/business-ledgers/assets/records/query",
            headers=headers,
...
def test_business_ledger_excel_preview_confirm_and_export(wm_paths) -> None:
    ...
    with TestClient(app) as client:
        headers = {"X-Agent-Bridge-User": "root"}
        client.post("/api/v1/business-ledgers", headers=headers, json={"ledger_key": "assets", "name": "资产", "fields": [{"field_key": "name", "name": "名称", "field_type": "text", "query_modes": ["contains"]}]})
        workbook = Workbook()
        workbook.active.append(["名称"])
        workbook.active.append(["Excel 资产"])
        content = BytesIO(); workbook.save(content)
        preview = client.post("/api/v1/business-ledgers/assets/imports/xlsx/preview", headers=headers, files={"file": ("assets.xlsx", content.getvalue())})
        assert preview.json()["rows"] == 1
        confirmed = client.post(f"/api/v1/business-ledgers/assets/imports/xlsx/{preview.json()['preview_id']}/confirm", headers=headers)
        assert confirmed.json()["imported"] == 1
        exported = client.get("/api/v1/business-ledgers/assets/exports/xlsx", headers=headers)
        assert exported.status_code == 200
        assert load_workbook(BytesIO(exported.content)).active.max_row == 2

        template = client.get("/api/v1/business-ledgers/assets/imports/xlsx/template", headers=headers)
