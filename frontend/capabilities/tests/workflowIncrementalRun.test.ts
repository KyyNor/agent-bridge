...
test('WorkflowView previews stale increments and gives completed tasks a force-full entry', () => {
  ...
  assert.match(tasksComposable, /function taskExecutionMode\(task: WorkflowTask\)/)
  assert.match(tasksComposable, /task\.status === 'stale' \|\| task\.needs_refresh/)
  assert.match(tasksComposable, /return 'incremental'/)
  assert.match(tasksComposable, /return 'force_full'/)
