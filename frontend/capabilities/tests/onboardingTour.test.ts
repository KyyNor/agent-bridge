...
test('onboarding controller separates sidebar replay from automatic per-user persistence', () => {
  ...
  assert.match(styles, /\.agent-bridge-tour-popover \.agent-bridge-tour-skip \{[^}]*background: var\(--secondary\);[^}]*border: 1px solid var\(--muted-foreground\);[^}]*box-shadow: var\(--shadow-btn\);/s)
