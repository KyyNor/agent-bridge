...
test('memory api methods use the expected endpoints and payloads', async () => {
  ...
  assert.equal(calls[3].url, '/memory/blocks/dev-memory/search?q=tool+recall&limit=5')
  assert.equal(calls[4].url, '/memory/blocks/dev-memory/timeline?limit=7&cursor=cursor-1')
  assert.equal(calls[5].url, '/capability-profiles/dev/memory')
  assert.equal(calls[6].url, '/capability-profiles/dev/memory')
  assert.equal(calls[6].init?.method, 'PUT')
  assert.equal(calls[6].init?.body, JSON.stringify({ block_key: null, enabled: true }))
})
