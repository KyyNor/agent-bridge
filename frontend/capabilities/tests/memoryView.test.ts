...
test('memory api methods use the expected endpoints and payloads', async () => {
  ...
  assert.equal(calls[0].url, '/api/v1/memory/blocks')
  assert.equal(calls[1].url, '/api/v1/memory/blocks')
  ...
  assert.equal(calls[2].url, '/api/v1/memory/blocks/dev-memory/health')
  assert.equal(calls[3].url, '/api/v1/memory/blocks/dev-memory/dashboard')
  assert.equal(calls[4].url, '/api/v1/memory/blocks/dev-memory/dashboard/start')
  assert.equal(calls[4].init?.method, 'POST')
  assert.equal(calls[5].url, '/api/v1/memory/blocks/dev-memory/dashboard/stop')
  assert.equal(calls[5].init?.method, 'POST')
  assert.equal(calls[6].url, '/api/v1/memory/blocks/dev-memory/dashboard/touch')
  assert.equal(calls[6].init?.method, 'POST')
  assert.equal(calls[7].url, '/api/v1/memory/blocks/dev-memory/search?q=tool+recall&limit=5')
  assert.equal(calls[8].url, '/api/v1/memory/blocks/dev-memory/timeline?limit=7&cursor=cursor-1')
  assert.equal(calls[9].url, '/api/v1/capability-profiles/dev/memory')
  assert.equal(calls[10].url, '/api/v1/capability-profiles/dev/memory')
