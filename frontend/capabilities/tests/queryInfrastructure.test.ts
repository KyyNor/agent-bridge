...
test('model evaluation only polls while an evaluation is active', () => {
  ...
  assert.match(evaluations, /queryKeys\.modelEvaluationRun/)
  assert.match(evaluations, /api\.getModelEvaluationRun/)
  assert.match(evaluations, /refetchInterval/)
  assert.match(evaluations, /run\.status === 'queued' \|\| run\.status === 'running'/)
  assert.match(evaluations, /router\.push\(`\/model-evaluations\/\$\{encodeURIComponent\(run\.run_id\)\}`\)/)
  assert.match(evaluations, /router\.replace\('\/model-evaluations'\)/)
