...
test('model evaluation only polls while an evaluation is active', () => {
  ...
  assert.match(evaluations, /const selectedRunId = ref\(''\)/)
  assert.match(evaluations, /runs\.value\.find\(run => run\.run_id === selectedRunId\.value\)/)
  assert.match(evaluations, /selectedRunId\.value = run\.run_id/)
