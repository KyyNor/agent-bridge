...
import type { WorkflowEdge, WorkflowValidationError } from '../../api/types'
...
const props = defineProps<{ edge: WorkflowEdge; locked?: boolean; referenceItems?: WorkflowReferenceItem[]; issues?: WorkflowValidationError[] }>()
  ...
  <section class="space-y-3 p-4">
    ...
    <template v-else>
      ...
      <template v-if="edge.condition">
        <div>
          <Input ref="fieldInput" :model-value="condition.field" placeholder="nodes.classify.output.category" :aria-invalid="Boolean(issueFor('field'))" @focusin="activeField = fieldInput" @update:model-value="change({ field: String($event) })" />
          <p v-if="issueFor('field')" class="mt-1 text-xs text-destructive">{{ issueFor('field')?.message }}</p>
        </div>
        <div>
          <Select :model-value="condition.operator" @update:model-value="changeOperator(String($event))"><SelectTrigger :aria-invalid="Boolean(issueFor('operator'))"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="equals">等于</SelectItem><SelectItem value="not_equals">不等于</SelectItem><SelectItem value="exists">存在</SelectItem><SelectItem value="not_exists">不存在</SelectItem><SelectItem value="contains">包含</SelectItem></SelectContent></Select>
