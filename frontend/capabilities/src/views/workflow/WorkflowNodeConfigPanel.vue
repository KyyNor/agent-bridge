...
const props = defineProps<{ node: WorkflowNode; scripts: ManagedScript[]; skills: SkillPrompt[]; backends: string[]; referenceItems?: WorkflowReferenceItem[] }>()
...
<template>
  <section class="space-y-3 border-l p-4">
    ...
    <template v-else>
      <div><label class="mb-1 block text-xs text-muted-foreground">提示词</label><Textarea ref="promptInput" :model-value="node.config.prompt" class="min-h-28" @focusin="activeField = promptInput" @update:model-value="config({ prompt: String($event) })" /></div>
