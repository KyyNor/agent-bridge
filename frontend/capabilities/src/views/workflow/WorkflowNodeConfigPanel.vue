...
const outputSchema = computed(() => deriveNodeOutputSchema(props.node, selectedScript.value))
...
<template>
  <section ref="sectionRef" class="space-y-3 p-4">
    ...
    <template v-if="node.type === 'get_task'">
      ...
      <SchemaFieldEditor :model-value="outputSchema" label="输出 JSON Schema" :disabled="true" />
    </template>
    <template v-else-if="node.type === 'script'">
      ...
      <SchemaFieldEditor v-if="selectedScript?.output_schema" :model-value="outputSchema" label="输出 JSON Schema" :disabled="true" />
      <p v-else class="text-xs text-muted-foreground">所选脚本未声明输出 JSON Schema，运行时输出字段不可预览。</p>
    </template>
