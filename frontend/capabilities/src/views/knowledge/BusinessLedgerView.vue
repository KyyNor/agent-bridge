...
async function deleteLedger() { if (!ledger.value || !await confirm({ title: '删除业务台账', description: `确定删除「${ledger.value.name}」及全部数据？`, destructive: true, confirmText: '删除' })) return; await api.deleteBusinessLedger(ledger.value.ledger_key); await router.push('/business-ledgers') }
...
<template>
  ...
  <template v-else>
    ...
    <template v-else-if="!isDetail">
      <Teleport defer to="#ph-actions"><Button @click="router.push('/business-ledgers/new')"><Plus :size="15" />新建台账</Button></Teleport>
      ...
      <Card v-if="ledgers.length"><CardContent class="p-0"><div class="overflow-x-auto"><table v-if="ledgers.length" class="w-full"><thead><tr class="border-b border-border"><th class="px-4 py-3 text-left text-xs font-medium text-muted-foreground">台账</th><th class="px-4 py-3 text-left text-xs font-medium text-muted-foreground">描述</th><th class="px-4 py-3 text-left text-xs font-medium text-muted-foreground">字段</th><th class="px-4 py-3 text-left text-xs font-medium text-muted-foreground">数据</th><th class="px-4 py-3 text-left text-xs font-medium text-muted-foreground" /></tr></thead><tbody><tr v-for="item in ledgers" :key="item.ledger_key" class="border-b border-border/60 last:border-b-0"><td class="px-4 py-3"><div class="text-sm font-medium">{{ item.name }}</div><div class="font-mono text-xs text-muted-foreground">{{ item.ledger_key }}</div></td><td class="max-w-[360px] px-4 py-3 text-sm text-muted-foreground"><div class="truncate">{{ item.description || '暂无描述' }}</div></td><td class="whitespace-nowrap px-4 py-3 text-sm">{{ item.fields.length }} 字段</td><td class="whitespace-nowrap px-4 py-3 text-sm">{{ item.record_count }} 行</td><td class="px-4 py-3"><div class="flex gap-2"><Button variant="outline" size="sm" class="h-8 text-xs" @click="router.push(`/business-ledgers/${item.ledger_key}`)">浏览</Button><Button variant="outline" size="sm" class="h-8 text-xs" @click="router.push(`/business-ledgers/${item.ledger_key}/edit`)">编辑</Button><Button variant="ghost" size="sm" class="h-8 gap-1.5 text-xs text-destructive" @click="deleteLedgerItem(item)"><Trash2 :size="14" />删除</Button></div></td></tr></tbody></table></div></CardContent></Card>
    </template>
    <main v-else-if="ledger" class="flex min-h-[calc(100vh-8rem)] min-w-0 flex-col space-y-4">
      <div class="flex shrink-0 flex-wrap items-start gap-3"><Button variant="ghost" size="sm" class="-ml-2 mt-0.5" @click="router.push('/business-ledgers')"><ArrowLeft :size="16" />返回业务台账</Button><div class="min-w-0 flex-1 border-l border-border pl-3"><h1 class="truncate text-lg font-semibold">{{ ledger.name }}</h1><p class="truncate text-xs text-muted-foreground">{{ ledger.ledger_key }}</p></div><div class="flex flex-wrap gap-2"><Button variant="outline" size="sm" @click="openPlaneDialog"><Settings2 :size="15" />能力平面</Button><Button variant="outline" size="sm" @click="router.push(`/business-ledgers/${ledger.ledger_key}/edit`)"><Pencil :size="15" />编辑定义</Button><Button variant="destructive" size="sm" @click="deleteLedger"><Trash2 :size="15" />删除</Button></div></div>
