...
<template>
  ...
  <template v-else>
    ...
    <Dialog v-model:open="showImportDialog"><DialogContent class="sm:max-w-[560px]"><DialogHeader><DialogTitle>导入 Excel</DialogTitle></DialogHeader><div class="space-y-4"><p class="text-sm text-muted-foreground">列名会按字段标识或字段名称自动匹配。请先预览并确认导入结果。</p><div class="rounded-md border border-border bg-muted/20 p-3"><label for="business-ledger-import-file" class="text-sm font-medium">选择 Excel 文件</label><input id="business-ledger-import-file" class="mt-2 h-9 w-full cursor-pointer rounded-sm border border-input bg-transparent px-2.5 py-1 text-sm file:mr-2 file:border-0 file:bg-transparent file:font-medium" type="file" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" :disabled="importing" @change="chooseFile" /><p class="mt-1.5 text-xs text-muted-foreground">仅支持 .xlsx 文件。</p></div><div v-if="importFile" class="text-xs text-muted-foreground">已选择：<span class="font-medium text-foreground">{{ importFile.name }}</span></div><div v-if="importPreview" class="rounded-md bg-muted p-3 text-sm">将导入 {{ importPreview.rows }} 行；错误 {{ importPreview.errors.length }} 行。<ul v-if="importPreview.errors.length" class="mt-2 list-disc pl-5 text-xs text-muted-foreground"><li v-for="item in importPreview.errors.slice(0, 5)" :key="item.row">第 {{ item.row }} 行：{{ item.error }}</li></ul></div></div><DialogFooter><Button variant="outline" @click="showImportDialog = false">取消</Button><Button v-if="importPreview" :disabled="importing" @click="confirmImport">{{ importing ? '导入中...' : '确认导入' }}</Button><Button v-else :disabled="!importFile || importing" @click="previewImport">{{ importing ? '预览中...' : '预览导入' }}</Button></DialogFooter></DialogContent></Dialog>
