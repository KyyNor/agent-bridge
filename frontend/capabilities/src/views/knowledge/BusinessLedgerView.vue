...
<template>
  ...
  <template v-else>
    ...
    <template v-else-if="!isDetail">
      ...
      <div v-if="ledgers.length" class="overflow-hidden rounded-lg border border-border bg-card"><button v-for="item in ledgers" :key="item.ledger_key" type="button" class="list-row-interactive flex w-full items-center gap-3 border-b border-border px-4 py-3 text-left last:border-b-0" @click="navigateTo(`business-ledgers/${item.ledger_key}`)"><Database :size="18" class="shrink-0 text-primary" /><div class="min-w-0 flex-1"><div class="flex min-w-0 items-center gap-2"><h2 class="truncate text-sm font-medium">{{ item.name }}</h2><span class="shrink-0 font-mono text-xs text-muted-foreground">{{ item.ledger_key }}</span></div><p class="mt-1 truncate text-xs text-muted-foreground">{{ item.description || '暂无描述' }}</p></div><div class="flex shrink-0 items-center gap-2"><Badge variant="secondary">{{ item.record_count }} 行</Badge><Badge variant="outline">{{ item.fields.length }} 字段</Badge></div></button></div>
    </template>
