...
<template>
  ...
  <main v-else class="flex min-h-[calc(100vh-8rem)] min-w-0 flex-col space-y-4">
    ...
    <div class="sticky bottom-0 flex justify-end gap-2 border-t border-border bg-background/95 py-4 backdrop-blur"><Button variant="outline" @click="navigateTo(props.ledgerKey ? `business-ledgers/${props.ledgerKey}` : 'business-ledgers')">取消</Button><Button :disabled="saving || readOnly" :title="readOnly ? SHARED_RESOURCE_READ_ONLY_HINT : undefined" @click="save">{{ saving ? '保存中...' : '保存' }}</Button></div>
