...
async function save(): Promise<boolean> {
  ...
  try {
    ...
    await router.push(`/business-ledgers/${definition.value.ledger_key}`)
    return true
...
<template>
  ...
  <main v-else class="flex min-h-[calc(100vh-8rem)] min-w-0 flex-col space-y-4">
    <div class="flex shrink-0 items-start gap-3">
      <Button variant="ghost" size="sm" class="-ml-2 mt-0.5" @click="router.push(props.ledgerKey ? `/business-ledgers/${props.ledgerKey}` : '/business-ledgers')"><ArrowLeft :size="16" />返回业务台账</Button>
    ...
    <div class="sticky bottom-0 flex justify-end gap-2 border-t border-border bg-background/95 py-4 backdrop-blur"><Button variant="outline" @click="router.push(props.ledgerKey ? `/business-ledgers/${props.ledgerKey}` : '/business-ledgers')">取消</Button><Button :disabled="saving" @click="save">{{ saving ? '保存中...' : '保存' }}</Button></div>
