...
import type { AccessActorContext, BusinessLedger, BusinessLedgerField, ResourceVisibility } from '../../api/types'
...
const loadedLedger = ref<BusinessLedger | null>(null)
const readOnly = computed(() => isSharedResourceReadOnly(actorContext.value, loadedLedger.value))
...
async function save(): Promise<boolean> {
  ...
  try {
    ...
    await routeTo(`business-ledgers/${definition.value.ledger_key}`)
    return true
...
onMounted(async () => {
  const [, actor] = await Promise.all([loadDefinition(), api.getAccessContext()])
...
<template>
  ...
  <main v-else class="flex min-h-[calc(100vh-8rem)] min-w-0 flex-col space-y-4">
    <div class="flex shrink-0 items-start gap-3">
      <Button variant="ghost" size="sm" class="-ml-2 mt-0.5" @click="routeTo(props.ledgerKey ? `business-ledgers/${props.ledgerKey}` : 'business-ledgers')"><ArrowLeft :size="16" />返回业务台账</Button>
      ...
      <Button variant="outline" size="sm" :disabled="designing || readOnly" :title="readOnly ? SHARED_RESOURCE_READ_ONLY_HINT : undefined" @click="openBusinessLedgerDesigner(isEdit ? 'modify' : 'create')"><WandSparkles :size="15" />AI 设计</Button>
    </div>
    <div v-if="readOnly" class="rounded-md border border-warning/30 bg-warning-soft px-3 py-3 text-sm text-warning-soft-fg">{{ SHARED_RESOURCE_READ_ONLY_HINT }}</div>
    <div v-if="error" class="rounded-lg bg-destructive-soft p-3 text-sm text-destructive">{{ error }}</div>
    <Card><CardContent class="grid gap-4 p-5 md:grid-cols-2"><label class="text-sm">标识<Input v-model="definition.ledger_key" :disabled="isEdit" placeholder="例如 asset_inventory" /></label><label class="text-sm">名称<Input v-model="definition.name" placeholder="例如 资产台账" /></label><label class="text-sm md:col-span-2">描述<Input v-model="definition.description" placeholder="说明这份台账包含什么数据" /></label><ResourceVisibilitySelect v-model="definition.visibility" hint="共享后所有小组可查询和导出，维护仍只允许归属小组。" /></CardContent></Card>
    <Card class="min-h-0 flex-1"><CardContent class="p-0"><div class="flex flex-wrap items-center justify-between gap-3 border-b border-border p-4"><div><h2 class="text-base font-medium">字段定义</h2><p class="mt-1 text-sm text-muted-foreground">数字、日期与日期时间字段支持范围筛选。</p></div><Button size="sm" variant="outline" @click="addField" :disabled="readOnly" :title="readOnly ? SHARED_RESOURCE_READ_ONLY_HINT : undefined"><Plus :size="14" />添加字段</Button></div><div class="overflow-x-auto"><table class="w-full min-w-[980px] text-sm"><thead><tr class="border-b text-left text-xs text-muted-foreground"><th class="p-3 font-medium">字段名称</th><th class="p-3 font-medium">字段标识</th><th class="p-3 font-medium">类型 / 枚举选项</th><th class="p-3 font-medium">字段属性</th><th class="p-3 font-medium">文本检索</th><th class="p-3" /></tr></thead><tbody><tr v-for="(field, index) in definition.fields" :key="index" class="border-b border-border/60 align-top"><td class="p-3"><Input v-model="field.name" placeholder="例如 IP 地址" :disabled="readOnly" /></td><td class="p-3"><Input v-model="field.field_key" placeholder="例如 ip_address" :disabled="readOnly" /></td><td class="space-y-2 p-3"><select :value="field.field_type" :disabled="readOnly" class="h-9 w-full rounded-md border border-input bg-background px-2" @change="changeFieldType(field, ($event.target as HTMLSelectElement).value)"><option value="text">文本</option><option value="number">数字</option><option value="enum">枚举</option><option value="date">日期</option><option value="datetime">日期时间</option></select><Input v-if="field.field_type === 'enum'" :model-value="field.enum_values.join(', ')" placeholder="选项用逗号分隔" :disabled="readOnly" @update:model-value="updateEnumValues(field, String($event))" /></td><td class="p-3"><div class="grid gap-2"><label class="flex items-center gap-2"><input v-model="field.required" type="checkbox" class="size-4 rounded" :disabled="readOnly" />必填</label><label class="flex items-center gap-2"><input v-model="field.agent_readable" type="checkbox" class="size-4 rounded" :disabled="readOnly" />允许 Agent 返回</label></div></td><td class="p-3"><label v-if="field.field_type === 'text'" class="flex items-center gap-2"><input type="checkbox" :checked="field.query_modes.includes('contains')" class="size-4 rounded" :disabled="readOnly" @change="toggleContains(field)" />支持模糊匹配</label><span v-else class="text-xs text-muted-foreground">—</span></td><td class="p-3"><Button size="sm" variant="ghost" :disabled="definition.fields.length === 1 || readOnly" :title="readOnly ? SHARED_RESOURCE_READ_ONLY_HINT : '删除字段'" @click="removeField(index)"><Trash2 :size="14" /></Button></td></tr></tbody></table></div></CardContent></Card>
    <div class="sticky bottom-0 flex justify-end gap-2 border-t border-border bg-background/95 py-4 backdrop-blur"><Button variant="outline" @click="routeTo(props.ledgerKey ? `business-ledgers/${props.ledgerKey}` : 'business-ledgers')">取消</Button><Button :disabled="saving || readOnly" :title="readOnly ? SHARED_RESOURCE_READ_ONLY_HINT : undefined" @click="save">{{ saving ? '保存中...' : '保存' }}</Button></div>
