...
名单维护在 ``<AGENT_BRIDGE_ROOT>/config/dsh-plugins.txt``：每行一个插件
spec（如 ``@scope/name@latest`` 或裸包名），空行与 ``#`` 注释忽略；文件
缺失或为空表示不安装任何插件，修改名单无需重启服务（每次按需读取）。

安装进度记录在 ``<DSH_HOME>/agent-bridge-plugins.txt``（已成功安装的
spec 列表，0600、归属目标 Linux 用户）：runtime 就绪后补装名单中尚未
记录的条目，全部就位后不再触盘——插件安装实际只发生在首次初始化，以及
名单后来新增条目时的一次补装；名单删除条目不会卸载已装插件。
"""
...
PLUGIN_PROFILE = "web"
# 名单文件（相对 AGENT_BRIDGE_ROOT/config）。
PLUGIN_LIST_FILENAME = "dsh-plugins.txt"
# 已安装 marker 文件（相对 DSH_HOME）。
PLUGIN_MARKER_FILENAME = "agent-bridge-plugins.txt"


def plugin_list_path(config_dir: Path) -> Path:
    return Path(config_dir) / PLUGIN_LIST_FILENAME
...
def read_plugin_list(config_dir: Path) -> list[str]:
    """读取插件名单；文件缺失、为空或不可读时返回空列表（不安装）。"""
    path = plugin_list_path(config_dir)
    try:
