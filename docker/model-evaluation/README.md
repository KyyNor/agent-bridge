...
当前 SWE Agent 单次模型请求上限为 6 分钟、最多 40 轮；主服务对每条 Agent 命令限制 3 分钟，对最终验收限制 15 分钟。没有单题总时长上限。HumanEval / MBPP 的代码生成阶段单次模型补全请求超时为 300 秒（`MODEL_TIMEOUT_SECONDS`）；单题生成失败（超时、网络或接口错误）只把该题补全置空并在结果中记录 `generation_error`，沙箱按失败计分后继续执行剩余题目，不会中止整批评估。修改 `agent-bridge-code-generate` 或 `code-sandbox` 后需要重建 `agent-bridge-opencompass-runner` / `agent-bridge-agent-worker` 镜像。
