...
「系统管理 → 模型评估」使用 OpenCompass 的 OpenAI 兼容 API 适配器，第一版提供 GSM8K 与 MATH 两个小规模 Demo 数据集。页面可设置“每个数据集最多题数”（默认 64、范围 1–1000），所有勾选数据集按相同上限执行，题数会随评估记录保存。评估模型的 URL 与 API Key 同时留空时，会复用「系统管理 → 公共模型配置」中的全量探测关键词模型连接；密钥仅传给评估子进程，不保存到 SQLite 或评估记录。
