...
知识库、MCP/OpenAPI 服务和代码仓库的新建请求均接受 `visibility=group|shared`，默认 `group`。后端在列表、详情、编辑、删除、MCP 工具执行和 CodeGraph 查询入口统一检查资源归属；能力 Profile 的 allow 列表与小组可见范围取交集，不能借助 Profile 绕过小组隔离。用户后来换组不会迁移既有资源，资源仍由创建时的归属组维护。
