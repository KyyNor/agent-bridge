...
- 历史数据清理统一走 `system_config/data_retention.py` 的每日生命周期任务（`retention_detail_days`/`retention_history_days`/`retention_cleanup_time` 存于 `knowledge_sync_config`，默认 20/60/22:00）：不得恢复写入路径上的 runtime log prune，表写入处不得附带清理触发；清理必须分批 DELETE + `wal_checkpoint`，VACUUM 只允许出现在 `2026_09_data_retention_v1` marker 守卫的首启迁移；主库 `data_retention_meta` 表是该迁移的幂等凭据，只能由迁移逻辑维护。业务台账、知识库/文档、权限、Script/Skill/Workflow 定义、当前工作流产物（`is_current=1`）与 `workflow_tasks` 永不按时间删除；清理历史工作流产物依赖 FTS 触发器与外键级联保持一致，护栏测试见 `tests/test_data_retention.py`。
