...
- DSH Workspace 代理目标只能来自当前登录业务用户已登记的 runtime 状态，禁止接受 URL 参数指定端口；HTTP/WS 转发复用 `dashboard_proxy` 骨架，新增响应头/查询串改写走 hook 而不是复制转发实现。首次导航的 `?token=` 只能在浏览器尚未持有该 authority 的 DSH 会话 Cookie 时补发（否则与 DSH 303 形成重定向循环），token 不得出现在浏览器地址栏。能力平面注入经 `dsh/workspace.py` 的短期 capability（用户唯一、TTL 受限）与 DSH loader patch 覆盖文件（`<DSH_HOME>/agent-bridge-mcp.patch.yml`，0600，经 `{patch}` → `--patch` 生效）；`/mcp` 校验后以业务用户身份沿用 Profile 权限体系；覆盖文件只写 DSH 配置目录，不进 `AGENT_BRIDGE_ROOT/data`。
