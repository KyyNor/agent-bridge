...
- DSH 插件首装：名单维护在 `<AGENT_BRIDGE_ROOT>/config/dsh-plugins.txt`（每行一个 spec，`#` 注释；缺失或为空则零开销），runtime 就绪后在后台线程以目标 Linux 用户身份执行 `dsh plugin --profile web add <spec>`，成功条目记入 `<DSH_HOME>/agent-bridge-plugins.txt`（0600）。实际只在首次初始化（及名单新增条目）执行；失败条目不写 marker、下次启动自动重试；名单移除条目不卸载已装插件；安装环境不得携带组级 API Key。
