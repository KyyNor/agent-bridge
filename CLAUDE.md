...
数据权限采用单小组归属模型：`access_users` 是可独立存在的用户目录，`access_groups` 是小组目录，`user_group_memberships` 仅将稳定用户 ID 映射到当前小组。用户可未分配小组；换组或取消归属只更新或删除关系，绝不删除用户。受保护资源保存 `owner_group_key` 和 `visibility=group|shared`。读取规则是同组或共享，写入规则始终是同组；`admins` 仅作为维护旁路。删除含成员的小组必须明确拒绝。存量资源优先按创建者映射归组，无法确认归属时保持组内范围并进入维护修复，不得自动共享。资源访问必须进入 `AccessControlService` 与资源 adapter，不在 API 路由或前端复制判断。
...
- 插件首装（`dsh/plugins.py`）：名单内置于包内 `src/agent_bridge/dsh/dsh-plugins.txt`
  ...
  （`allowBuilds` map，`BLOCKED_BUILDS` 当前为 node-pty；pnpm 10.33 正式键，
  等价于 `ignoredBuiltDependencies`，并会清理历史放行条目）：pnpm 10 默认拦截
  依赖构建并留待定告警，node-pty 又无 Linux 预编译、构建需工具链与 Node
...
工作流编辑器对归属组用户开放，表单路由的首屏依赖必须全部是登录即可读的接口：脚本、技能、能力平面列表，以及 `GET /api/v1/agent-runtime/backends`（默认后端 + 各后端 slug/展示名/能力/思考力度，由 `_available_agent_backends_payload` 投影，不含命令与模型配置）。管理员专用配置接口（`/agent-runtime/config`、`/sync-config`、`/claude-mem/config` 等）只服务系统管理页，不得进入业务页面加载链路；工作流写入权限始终由 `owner_group_key` 决定。
...
知识库的后端目标在 `backend_targets.config_json.sync_on_upload` 中分别控制管理后台上传完成后是否立即同步，默认 `false`；`knowledge_bases.sync_on_upload` 仅保留兼容旧调用。关闭时文档只入库并进入同步队列，由 `doc_sync_cron` 处理；上传队列只能对已启用的后端目标调用立即同步接口。HTTP 文档上传的 `later` 默认值必须保持为延后同步。
